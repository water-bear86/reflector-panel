        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-4 flex flex-nowrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <div className="hidden sm:flex items-center gap-1.5">
              ...
              <a href={`https://solscan.io/token/${STIMMY.mint}`} target="_blank" rel="noopener noreferrer" title="Explorer" aria-label="Solscan Explorer" className="w-9 h-9 flex items-center justify-center rounded-none bg-surface-700 border border-white/[0.04] hover:bg-surface-600 transition-colors">
                <ScanIcon className="w-5 h-5" />
              </a>
              <a href="https://github.com/YATSPAT/YATSPAT" target="_blank" rel="noopener noreferrer" title="GitHub (YATSPAT)" aria-label="GitHub Repository" className="w-9 h-9 flex items-center justify-center rounded-none bg-surface-700 border border-white/[0.04] text-slate-200 hover:text-white transition-colors">
                    ...
                    {rules.map((rule, i) => (
                      <div key={i} className="rounded-none border border-white/[0.05] bg-surface-900/60 p-3 space-y-3">
                        ...
                        {rule.type === "distribute" && (
                          <>
                            <input
                              aria-label="Holder mint address"
                        ...
                        {rule.type === "buy-burn" && (
                          <input
                            aria-label="Token mint to buy back & burn"
                            className="glass-input font-mono text-xs"
                            value={rule.targetMint || ""}
                            onChange={(e) => updateRule(i, { targetMint: e.target.value })}
