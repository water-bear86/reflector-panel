            ...
            {!deployResult?.ok && (
              ...
              >
                    ...
                    {rules.map((rule, i) => (
                      <div key={i} className="rounded-none border border-white/[0.05] bg-surface-900/60 p-3 space-y-3">
                        <div className="flex items-center gap-2">
                          ...
                          {rules.length > 1 && (
                            <button type="button" onClick={() => removeRule(i)} className="text-xs text-rose-400 hover:text-rose-300 px-2 shrink-0" aria-label={`Remove rule ${i + 1}`}>
                              Remove
                            </button>
                          )}
                        </div>
                        ...
                        {rule.type === "buy-burn" && (
                          <input
                            ...
                            aria-label={`Rule ${i + 1} buyback and burn token mint`}
                          />
                        )}
                        {rule.type === "send" && (
                          <input
                ...
                <div data-tour="poll-interval">
                  <label id="poll-interval-label" className="block text-xs text-slate-400 mb-1.5">Check interval</label>
              ...
              <div className="glass-card p-8 space-y-5">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      className={`dazzle-copy btn-secondary text-xs shrink-0 ${walletCopied ? "dazzle-copied" : ""}`}
                      onClick={copyWallet}
